...
(
    ...
    player_class    VARCHAR(20) NOT NULL DEFAULT 'SABER' COMMENT '职阶:SABER/ARCHER/CASTER',
...
--     money        BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '金币',
    ...
    battle_order    INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '当前遇敌顺序序号',
...
(
    ...
    save_id                BIGINT UNSIGNED NOT NULL COMMENT '存档ID',
--     user_id                BIGINT UNSIGNED NOT NULL COMMENT '用户ID',
    monster_id             BIGINT UNSIGNED NOT NULL COMMENT '魔物ID',
    player_current_hp      INT      NOT NULL DEFAULT 0 COMMENT '角色当前HP',
    player_current_mp      INT      NOT NULL DEFAULT 0 COMMENT '角色当前MP',
    player_current_defend  INT      NOT NULL DEFAULT 0 COMMENT '角色当前格挡值',
...
--     target_num         INT          NOT NULL DEFAULT 0 COMMENT '可作用对象数量(单体/AOE)',
    ...
    cd                 INT          NOT NULL DEFAULT 0 COMMENT '技能冷却回合数',
    create_time        DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time        DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
...
(
    id                  BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    battle_id           BIGINT UNSIGNED NOT NULL COMMENT '对应的战斗ID',
    player_id           BIGINT UNSIGNED NOT NULL COMMENT '角色ID',
    action_id           BIGINT UNSIGNED NOT NULL COMMENT '技能ID',
...
(
    ...
    monster_id          BIGINT  UNSIGNED NOT NULL COMMENT '魔物ID',
    -- todo:楼层进度奖励用作未来随机化遇敌功能
    reward_progress     INT      NOT NULL DEFAULT 0 COMMENT '击败后获得的层数进度(0-100)',
    create_time         DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time         DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    UNIQUE KEY          uk_floor_monster_order (floor, battle_order, monster_id) COMMENT '楼层+怪物+顺序唯一索引，防止重复配置'
) ENGINE = InnoDB
    DEFAULT CHARSET = utf8mb4
    COMMENT = '魔塔-楼层魔物配置表';
